import { BaseRunners, PlaySnapshot, ScoringPlay, ScoringPlayType, ScoringPlaySummary, SacBuntOutcome, SacBuntSummary } from './types';
import { classifyPlay, parseBatterAction, parseRunnerSubEvent, getPlayerNameFromText } from './parse-play';
...
export function extractScoringPlays(
  ...
  if (playType === 'plate_appearance') {
    ...
    if ((batterAction.result === 'walk' || batterAction.result === 'hbp') &&
        basesBefore.first && basesBefore.second && basesBefore.third) {
...
function mapBatterResultToScoringType(
  ...
  switch (batterResult) {
    ...
    case 'bunt_single': return 'bunt_single';
    case 'walk': return 'walk';
    case 'hbp': return 'hbp';
    case 'sac_fly': return 'sac_fly';
    case 'sac_bunt': return 'sac_bunt';
    case 'reached': {
      ...
      if (runnerLower.includes('error') || batterLower.includes('error')) return 'error';
...
export function computeSacBuntOutcomes(snapshots: PlaySnapshot[], opponent: string, url: string): SacBuntOutcome[] {
  ...
  for (let i = 0; i < snapshots.length; i++) {
    ...
    const subEvents = snap.playText.replace(/\.$/, '').split(';').map(s => s.trim());
    const batterAction = parseBatterAction(subEvents[0]);
    ...
    for (let j = i; j < snapshots.length; j++) {
      ...
      for (const sp of snapshots[j].scoringPlays) {
        if (sp.runnerName && runnersOnBase.includes(sp.runnerName) && !runnersScored.includes(sp.runnerName)) {
