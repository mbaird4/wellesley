    ...
    <div class="mt-3 flex flex-col gap-2.5">
      ...
      @for (row of rows(); track row.label) {
        <div class="flex items-center gap-3">
          ...
          <div
            class="bg-surface-sunken relative flex h-7 min-w-0 flex-1 items-center overflow-hidden rounded-md"
          >
            <div
              class="bg-chart-bar absolute inset-y-0 left-0 rounded-md transition-all duration-300"
              [style.width.%]="row.barWidthPct"
            ></div>
            <span
              class="text-content-bright relative z-10 pl-2 text-xs font-semibold tabular-nums"
            >
              {{ row.count }}
            </span>
          </div>
