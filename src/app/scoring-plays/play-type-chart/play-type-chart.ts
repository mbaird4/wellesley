  ...
  template: `
    <div class="flex flex-col gap-1.5">
      ...
      <div class="flex items-center gap-3 px-1 py-1">
        <button class="text-content-dim hover:text-content-muted w-28 shrink-0 cursor-pointer border-none bg-transparent text-right text-xs font-semibold tracking-widest uppercase transition-colors" (click)="sort('value')">Type</button>
      ...
      @for (row of rows(); track row.type) {
        <div class="group flex items-center gap-3">
          ...
          <div class="relative flex h-7 min-w-0 flex-1 items-center">
            <div class="absolute inset-y-0 left-0 rounded-r-md transition-all duration-300" [class]="row.barColorClass" [style.width.%]="row.barWidthPct"></div>
            <span class="text-content-bright relative z-10 pl-2 text-sm font-semibold tabular-nums">
              {{ row.count }}
            </span>
          </div>
