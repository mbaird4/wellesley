...
export function buildStats(p: SprayPrintPlayerSummary): StatCell[] {
  ...
  return [
    ...
    { label: 'SLG', value: fmtAvg(p.slg), flagged: hasRates && slg >= HIGH_SLG },
    { label: 'wOBA', value: fmtAvg(p.woba), flagged: false },
    { label: 'PA', value: pa > 0 ? String(pa) : '—', flagged: false },
    { label: 'H', value: p.h !== undefined ? String(p.h) : '—', flagged: false },
    { label: 'XBH', value: String(xbh), flagged: xbh >= MIN_XBH },
    { label: 'BB', value: p.bb !== undefined ? String(p.bb) : '—', flagged: hasRates && bbRate >= HIGH_BB_RATE },
    { label: 'K', value: p.so !== undefined ? String(p.so) : '—', flagged: hasRates && (kRate >= HIGH_K_RATE || kRate <= LOW_K_RATE) },
    { label: 'RBI', value: p.rbi !== undefined ? String(p.rbi) : '—', flagged: hasRates && rbiRate >= HIGH_RBI_RATE },
    {
      label: 'SB',
