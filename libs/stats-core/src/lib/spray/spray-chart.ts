...
const DIRECTION_MAPPINGS: DirectionMapping[] = [
  ...
  { pattern: /down the rf line|to right field line/i, zone: 'rf_line', angle: 10, isInfield: false },
  { pattern: /down the lf line|to left field line/i, zone: 'lf_line', angle: 170, isInfield: false },
  { pattern: /to right center|to rc\b/i, zone: 'rf_cf', angle: 55, isInfield: false },
  { pattern: /to left center|to lc\b/i, zone: 'lf_cf', angle: 125, isInfield: false },
  { pattern: /through the right side/i, zone: 'rf_cf', angle: 50, isInfield: false },
  { pattern: /through the left side/i, zone: 'lf_cf', angle: 130, isInfield: false },
  { pattern: /up the middle/i, zone: 'cf', angle: 90, isInfield: false },

  // Outfield — general field areas
  { pattern: /to right field\b|to rf\b/i, zone: 'rf', angle: 30, isInfield: false },
  { pattern: /to center field\b|to cf\b/i, zone: 'cf', angle: 90, isInfield: false },
  { pattern: /to left field\b|to lf\b/i, zone: 'lf', angle: 150, isInfield: false },

  // Infield positions
  { pattern: /to third base|to 3b\b/i, zone: 'if_3b', angle: 155, isInfield: true },
  { pattern: /to shortstop|to ss\b/i, zone: 'if_ss', angle: 120, isInfield: true },
  { pattern: /to second base|to 2b\b/i, zone: 'if_2b', angle: 75, isInfield: true },
  { pattern: /to first base|to 1b\b/i, zone: 'if_1b', angle: 30, isInfield: true },
  { pattern: /to pitcher\b|to p\b|back to the pitcher/i, zone: 'if_p', angle: 90, isInfield: true },
...
export function parseBuntZone(playText: string): {
  ...
  if (outAtFirstMatch) {
    ...
    if (mapping) {
      return { zone: mapping.zone, angle: mapping.angle, direction: `fielded by ${fielder}` };
    }
  }
  ...
  if (fielderMatch) {
    ...
    if (mapping) {
      return { zone: mapping.zone, angle: mapping.angle, direction: direction.direction };
    }
  }
...
export function parseSprayData(
  ...
  return snapshots
    ...
    .map((snap): SprayDataPoint | null => {
      ...
      const contactType = HIT_RESULTS.includes(result) && baseContactType !== 'bunt'
        ? 'hit'
        : baseContactType;
...
const ALL_ZONES: SprayZone[] = [
  'lf_line', 'lf', 'lf_cf', 'cf', 'rf_cf', 'rf', 'rf_line',
  'if_3b', 'if_ss', 'if_2b', 'if_1b', 'if_p', 'if_c',
  'plate_3b', 'plate_p', 'plate_1b',
];
